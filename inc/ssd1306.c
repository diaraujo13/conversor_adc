...
// Função para desenhar uma borda
void ssd1306_draw_border(ssd1306_t *ssd, uint8_t style)
{
  switch (style)
  {
  case 0:
  ...
  case 1:
    ...
    break;
  case 2:
    ssd1306_hline(ssd, 0, ssd->width - 1, 0, true);
    ssd1306_hline(ssd, 0, ssd->width - 1, ssd->height - 1, true);
    ssd1306_vline(ssd, 0, 0, ssd->height - 1, true);
    ssd1306_vline(ssd, ssd->width - 1, 0, ssd->height - 1, true);
    break;
  case 3:
    ssd1306_hline(ssd, 0, ssd->width - 1, 0, true);
    ssd1306_hline(ssd, 0, ssd->width - 1, ssd->height - 1, true);
    ssd1306_vline(ssd, 0, 0, ssd->height - 1, true);
    ssd1306_vline(ssd, ssd->width - 1, 0, ssd->height - 1, true);
    ssd1306_rect(ssd, 1, 1, ssd->width - 2, ssd->height - 2, false, false);
    break;
  }
}
...
void ssd1306_draw_square(ssd1306_t *ssd, uint8_t x, uint8_t y, uint8_t size, bool value)
{
